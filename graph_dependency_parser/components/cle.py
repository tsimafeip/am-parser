...
def cle_decode(scores, lengths):
    ...
    :param scores torch.Tensor of shape (batch_size,tokens, tokens);
        the length of the sentences is an array of length batch_size that specifies how long the sentences are
    :param lengths: actual lengths of the sentences, tensor of shape (batch_size,)
    :return: a tensor of shape (batch_size, tokens) that contains the heads of the tokens.
                Positions that go over the sentence length are filled with -1.
    """
    heads = []
...
def cle_loss(scores: torch.Tensor, lengths : torch.Tensor, gold_heads : torch.Tensor, normalize_wrt_seq_len : bool):
    """
    Parses a batch of sentences and computes a hinge loss
        (see code by Eliyahu Kiperwasser: https://github.com/elikip/bist-parser)
    :param scores torch.Tensor of shape (batch_size,tokens, tokens);
        the length of the sentences is an array of length batch_size that specifies how long the sentences are
    ...
    for m, g, l in zip(scores, gold_heads, lengths):
        ...
        my_range = np.arange(l)
        # remove padding at the end:
        m = m[:l, :l]
        g = g[:l]  # -> shape (l,)

        # make gold solution look worse by cost augmentation
        # (in the original, make non-gold look better)/introduce margin:
        # TODO is this right? PyTorch 1.12 throws a RuntimeError for in-place modification of a view
        m[my_range, g] -= 1.0  # cost augmentation

        r, _ = chu_liu_edmonds(m.detach().double().numpy())  # discard _score_ of solution, -> r has shape (l,)
        # this implementation says that head of artificial root is -1, but the rest of the pipeline says
        # the head of the artificial root is the artificial root itself (i.e. 0):
        r[0] = 0
        r = np.array(r)

        scores_of_solution = m[my_range, r]  # extract the scores belonging to the decoded edges -> shape (l,)
        scores_of_gold = m[my_range, g]  # extract the scores belonging to the gold edges -> shape (l,)
...
def find_root(heads: List[int], best_supertags: List[int], label_scores: np.array, root_edge_label_id: int,
              bot_id: int, modify: bool) -> Tuple[List[int], int]:
    """
    Selects the root and potentially changes some attachments.
    We take everything attached to the artificial root (index 0) and regroup it under the actual root.
    ...
    # those positions that are attached to 0 and whose best supertag is not \bot. If the supertag is bot, then we only
    # look at those that are heads themselves
